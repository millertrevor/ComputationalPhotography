...
def main():
  ...
  for dirname, dirnames, filenames in os.walk(imagesfolder0):
    for filename in filenames:
      ...
      if ext in exts and 'rbreverse' not in name:
        print "Reversing R and B in image {}.".format(filename)

        img = cv2.imread(os.path.join(dirname, filename))
        reversedimage = rbflip(img)
        cv2.imwrite(os.path.join(dirname,name+'RBreversed'+ext),reversedimage)
'''
